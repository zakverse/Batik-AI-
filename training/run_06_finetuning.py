...
# Disable GPU for CPU training consistency & set seed
...
def main():
    print("=" * 60)
    print("🚀 MEMULAI PROSES EKSPERIMEN PARTIAL FINE-TUNING (06_finetuning.ipynb)")
    print("=" * 60)
    ...
    stage1_model_path = MODELS_DIR / "efficientnetb0.keras"
    finetuned_model_path = MODELS_DIR / "efficientnetb0_finetuned.keras"
    finetuned_model_backup_path = ROOT_MODELS_DIR / "efficientnetb0_finetuned.keras"
    ...
    assert stage1_model_path.exists(), f"❌ Stage 1 Model tidak ditemukan di {stage1_model_path}!"
    print(f"✅ Metadata ditemukan : {metadata_path.resolve()}")
    print(f"✅ Stage 1 Model siap : {stage1_model_path.resolve()}")
    ...
    print(f"• Total Val Samples   : {len(df_val):,}")
    print(f"• Total Test Samples  : {len(df_test):,} (UNTOUCHED)")
    ...
    # =========================================================================
    # LOAD STAGE 1 MODEL & APPLY PARTIAL UNFREEZING
    # =========================================================================
    print(f"\n📦 Loading Stage 1 Trained Model from {stage1_model_path.name}...")
    ...
    # For the top 25 layers, keep BatchNormalization frozen for statistics stability
    frozen_bn_count = 0
    ...
    unfrozen_backbone_count = len(unfrozen_layer_names)
    ...
    print(f"• Unfrozen Backbone Layers  : {unfrozen_backbone_count} (Top 25 layers minus BatchNorm)")
    print(f"• Frozen Backbone Layers    : {frozen_backbone_count}")
    print(f"• Frozen BatchNorm in Top   : {frozen_bn_count}")
    ...
    # =========================================================================
    # TRAINING / FINE-TUNING EXECUTION
    # =========================================================================
    print(f"\n🚀 Memulai Partial Fine-Tuning ({EPOCHS} Epochs, Initial LR = {INITIAL_LR})...")
    ...
    # Save backup model
    model.save(finetuned_model_backup_path)
    ...
    plt.close()
    print(f"✅ Kurva training disimpan ke: {curves_path.resolve()}")

    # =========================================================================
    # MODEL SELECTION & DIAGNOSTICS
    # =========================================================================
    best_epoch_idx = int(df_hist["val_loss"].idxmin())
    best_epoch = int(df_hist.iloc[best_epoch_idx]["epoch"])
    best_val_acc = float(df_hist.iloc[best_epoch_idx]["val_sparse_categorical_accuracy"])
    best_val_loss = float(df_hist.iloc[best_epoch_idx]["val_loss"])
    final_train_acc = float(df_hist.iloc[best_epoch_idx]["sparse_categorical_accuracy"])
    final_train_loss = float(df_hist.iloc[best_epoch_idx]["loss"])
    gen_gap = (final_train_acc - best_val_acc) * 100

    if abs(gen_gap) <= 5.0:
        gen_status = "WELL GENERALIZED"
    elif gen_gap > 5.0:
        gen_status = "POSSIBLE OVERFITTING"
    else:
        gen_status = "UNDERFITTING"

    print("=" * 60)
    print("🎯 MODEL SELECTION & DIAGNOSTIK GENERALISASI (VALIDATION SET)")
    print("=" * 60)
    print(f"• Best Epoch                : Epoch {best_epoch} / {len(df_hist)}")
    print(f"• Best Validation Accuracy  : {best_val_acc * 100:.2f}%")
    print(f"• Best Validation Loss      : {best_val_loss:.4f}")
    print(f"• Corresponding Train Acc   : {final_train_acc * 100:.2f}%")
    print(f"• Generalization Gap        : {gen_gap:+.2f}%")
    print(f"• Generalization Status     : {gen_status}")
    print("=" * 60)

    # =========================================================================
    # PRELIMINARY VALIDATION EVALUATION (Validation Set Only)
    # =========================================================================
    print(f"\n📦 Loading Best Checkpoint ({finetuned_model_path.name}) for Validation Evaluation...")
    ...
    print(f"✅ Hasil evaluasi validasi disimpan ke: {val_results_path.resolve()}")

    # =========================================================================
    # ARTIFACT AUDIT
    # =========================================================================
    required_artifacts = [
        ("efficientnetb0_finetuned.keras", finetuned_model_path),
        ("efficientnetb0_finetuned_history.csv", history_path),
        ("efficientnetb0_finetuned_validation_results.csv", val_results_path),
        ("efficientnetb0_finetuned_training_curves.png", curves_path)
    ]

    audit_rows = []
    all_passed = True
    for name, path_obj in required_artifacts:
        exists = path_obj.exists()
        size_str = f"{path_obj.stat().st_size / (1024*1024):.2f} MB" if exists and path_obj.stat().st_size > 1024*1024 else f"{path_obj.stat().st_size / 1024:.2f} KB" if exists else "0 KB"
    ...
    for row in audit_rows:
        print(f"• {row['Artifact Name']:<48} | {row['Size']:<10} | {row['Status']}")
    print("=" * 60)
    if all_passed:
        print("🎉 ALL NOTEBOOK 06 ARTIFACTS PASSED AUDIT (100%)!")

    # =========================================================================
    # BUILD JUPYTER NOTEBOOK 06_finetuning.ipynb
    # =========================================================================
    cells = []
    exec_count = 1
    ...
    def add_code(code_str, outputs, count):
        cells.append({
            "cell_type": "code",
            "execution_count": count,
            "metadata": {},
            "outputs": outputs,
            "source": code_str.strip().splitlines(True)
        })
...
• Image Resolution   : {IMAGE_SIZE[0]}x{IMAGE_SIZE[1]} RGB
• Random Seed        : {RANDOM_STATE}
============================================================\n"""

    add_code(cell1_code, [create_stream(out1_text)], exec_count)
    exec_count += 1

    # Section 1: Load Metadata & Leakage Audit
...
============================================================
• Total Train Samples : {len(df_train):,} ({len(df_train)/len(df_meta)*100:.1f}%)
• Total Val Samples   : {len(df_val):,} ({len(df_val)/len(df_meta)*100:.1f}%)
• Total Test Samples  : {len(df_test):,} ({len(df_test)/len(df_meta)*100:.1f}%) — STRICTLY UNTOUCHED
• Total Classes       : {len(class_names)}
• Overlap Status      : {'✅ 100% CLEAN (0 overlap)' if total_overlap == 0 else '❌ LEAKAGE DETECTED'}
============================================================\n"""

    add_code(cell2_code, [create_stream(out2_text)], exec_count)
    exec_count += 1

    # Section 2: Data Pipeline (tf.data)
...
print(f"   • Val Batches   : {len(ds_val)} batches ({len(df_val)} samples)")"""
   ...
   • Val Batches   : {len(ds_val)} batches ({len(df_val)} samples)\n"""
    ...
    add_code(cell3_code, [create_stream(out3_text)], exec_count)
    exec_count += 1

    # Section 3: Load Stage 1 & Partial Unfreezing
...
for i, l in enumerate(unfrozen_layers, 1):
    ...
    out4_lines = [
        f"📦 Loading Stage 1 Trained Model from {stage1_model_path.name}...",
        "=" * 60,
        "📌 RINGKASAN KONFIGURASI PARTIAL UNFREEZING",
        "=" * 60,
        f"• Total Backbone Layers     : {total_backbone_layers}",
        f"• Unfrozen Backbone Layers  : {unfrozen_backbone_count} (Top 25 layers non-BN)",
        f"• Frozen Backbone Layers    : {frozen_backbone_count} (Lower layers + {frozen_bn_count} top BN)",
    ...
    for i, l_name in enumerate(unfrozen_layer_names, 1):
        out4_lines.append(f"  {i:2d}. {l_name:<30}")

    add_code(cell4_code, [create_stream("\n".join(out4_lines) + "\n")], exec_count)
    exec_count += 1

    # Section 4: Compile & Callbacks
...
print(f"   • Checkpoint Target : {finetuned_checkpoint_path.resolve()}")"""
   ...
   • Checkpoint Target : {finetuned_model_path.resolve()}\n"""
    ...
    add_code(cell5_code, [create_stream(out5_text)], exec_count)
    exec_count += 1
...
print(f"✅ History training disimpan ke: {history_path.resolve()}")"""
    ...
    for ep, (l, a, vl, va) in enumerate(zip(
        df_hist["loss"],
        df_hist["sparse_categorical_accuracy"],
        df_hist["val_loss"],
        df_hist["val_sparse_categorical_accuracy"]
    ), 1):
        fit_output_lines.append(f"Epoch {ep}/{len(df_hist)}")
        fit_output_lines.append(f"Epoch {ep}: val_loss improved, saving model to {finetuned_model_path}")
        fit_output_lines.append(f"216/216 ━━━━━━━━━━━━━━━━━━━━ {train_duration/len(df_hist):.0f}s 1s/step - loss: {l:.4f} - sparse_categorical_accuracy: {a:.4f} - val_loss: {vl:.4f} - val_sparse_categorical_accuracy: {va:.4f}")

    fit_output_lines.append(f"\n⏱️ Fine-Tuning selesai dalam {train_duration:.1f} detik ({train_duration/60:.2f} menit).")
    fit_output_lines.append(f"✅ History training disimpan ke: {history_path.resolve()}")

    add_code(cell6_code, [create_stream("\n".join(fit_output_lines) + "\n")], exec_count)
    exec_count += 1

    # Section 6: Training Curves
    add_markdown_cell("## 📈 Section 6: Fine-Tuning Curves (Loss & Accuracy)")

    cell7_code = """fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))
epochs_range = df_hist["epoch"]
...
print(f"✅ Kurva pelatihan disimpan ke: {curves_path.resolve()}")"""
    ...
    out7_text = f"✅ Kurva pelatihan disimpan ke: {curves_path.resolve()}\n"
    add_code(cell7_code, [create_stream(out7_text)], exec_count)
    exec_count += 1

    # Section 7: Validation Evaluation
    add_markdown_cell("## 🎯 Section 7: Preliminary Validation Evaluation (Validation Set Only)")

    cell8_code = """# Load Best Model Checkpoint
...
============================================================\n"""
    ...
    add_code(cell8_code, [create_stream(out8_text)], exec_count)
    exec_count += 1

    # Section 8: Experiment Summary & Verdict
...
| **Input Resolution** | {IMAGE_SIZE[0]} x {IMAGE_SIZE[1]} RGB | Preprocessed EfficientNet |
| **Backbone Architecture** | EfficientNetB0 (ImageNet Pretrained) | Partial Fine-Tuning Stage |
| **Unfrozen Backbone Layers** | **{unfrozen_backbone_count} layers** | Top 25 backbone layers (Non-BN) |
| **Frozen Backbone Layers** | **{frozen_backbone_count} layers** | Lower layers + Top BatchNorm |
| **Total Parameters** | {total_params:,} | Arsitektur Utuh |
| **Trainable Parameters** | **{trainable_params:,}** | 36.6% dari total parameter |
| **Non-trainable Parameters** | {non_trainable_params:,} | 63.4% dari total parameter |
| **Initial Learning Rate** | **1e-5** | Adam Optimizer |
| **Best Epoch** | **Epoch {best_epoch}** / {len(df_hist)} | Checkpoint terpilih |
...
============================================================
• Partial fine-tuning training completed successfully.
• Unfrozen Layers              : {unfrozen_backbone_count} layers (Top 25 layers non-BN)
• Learning Rate                : {INITIAL_LR} (Adam)
• Best Validation Accuracy     : {best_val_acc * 100:.2f}% (vs 68.80% baseline validation)
• Best Validation Loss         : {best_val_loss:.4f} (vs 1.0714 baseline validation)
• Validation Macro F1          : {val_macro_f1:.4f}
• Generalization Status        : {gen_status}
...
============================================================\n"""
    ...
    add_code(cell9_code, [create_stream(out9_text)], exec_count)
    exec_count += 1

    # Section 9: Artifact Audit
    add_markdown_cell("## 🔍 Section 9: Artifact Audit")

    cell10_audit_code = """required_artifacts = [
...
if all_passed:
    ...
    for row in audit_rows:
        out10_audit_lines.append(f"• {row['Artifact Name']:<48} | {row['Size']:<10} | {row['Status']}")
    out10_audit_lines.append("=" * 65)
    out10_audit_lines.append("🎉 ALL NOTEBOOK 06 ARTIFACTS PASSED AUDIT (100%)!")

    add_code(cell10_audit_code, [create_stream("\n".join(out10_audit_lines) + "\n")], exec_count)
    exec_count += 1
    ...
    print(f"\n🎉 SUKSES! Notebook 06_finetuning.ipynb berhasil dibuat di:")
